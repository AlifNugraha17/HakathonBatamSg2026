...
export function useLokaBatamStore() {
  ...
  // Filtered Destinations based on region, category, search
  ...
  const loadSalonsFromApi = async () => {
    try {
      ...
      console.warn('[LokaBatam Store] Error loading destinations from database:', e.message);
    } finally {
  ...
  const loadBookingsFromApi = async () => {
    try {
      ...
      console.warn('[LokaBatam Store] Error loading bookings from database:', e.message);
    }
  };
  ...
  const toggleSaveSalon = (salonId) => {
    ...
    if (idx > -1) {
      ...
      showInfo({
        id: 'Destinasi dihapus dari daftar bookmark.',
        en: 'Destination removed from bookmarks.'
      }, {
        id: 'Bookmark',
        en: 'Bookmark'
      });
      ...
      showSuccess({
        id: 'Destinasi berhasil disimpan ke bookmark!',
        en: 'Destination saved to bookmarks!'
      }, {
  ...
  const addBooking = async (newBooking) => {
    try {
      const created = await api.createBooking({
        ...
        service_name: newBooking.serviceName || newBooking.service_name || 'Destination Visit',
        therapist_name: newBooking.therapistName || newBooking.therapist_name || 'Lead Specialist',
      ...
      const code = created?.booking_code || created?.bookingCode || created?.id || 'LOKA-SG';
      showSuccess({
      ...
      const fallbackBooking = {
        ...
        bookingCode: `LOKA-${Math.floor(1000 + Math.random() * 9000)}`,
        spa_id: newBooking.salonId || '1',
        salonId: newBooking.salonId || '1',
        salonName: newBooking.salonName || 'LokaBatam Destination Partner',
        serviceName: newBooking.serviceName || 'Destination Experience',
        guestName: newBooking.guestName || 'Marcus Lim',
        therapistName: newBooking.therapistName || 'Dr. Hendra / Specialist',
  ...
  const createBookingFromSlot = async (slot = {}) => {
    const bookingItem = {
      ...
      serviceName: slot.serviceName || 'Specialist Session',
      therapistName: slot.therapistName || 'Lead Practitioner',
  ...
  const confirmBooking = async (bookingId) => {
    ...
    try {
      localStorage.setItem('lokabatam_confirmed_orders_' + bookingId, 'confirmed');
    } catch (e) {}
  ...
  const declineBooking = async (bookingId) => {
    ...
    try {
      localStorage.setItem('lokabatam_confirmed_orders_' + bookingId, 'cancelled');
    } catch (e) {}
  ...
  const toggleFlashSlot = async (slotId) => {
    try {
      ...
      showInfo({
        id: 'Status ketersediaan slot berhasil diperbarui.',
        en: 'Flash slot availability updated.'
      }, {
        id: 'Slot Destinasi',
        en: 'Destination Slot'
      });
      ...
      showError({
        id: e.message || 'Gagal memperbarui status slot.',
        en: e.message || 'Failed to update slot status.'
      }, {
        id: 'Update Gagal',
        en: 'Update Failed'
      });
  ...
  const addMerchantFlashSlot = async (newSlot) => {
    try {
      const created = await api.broadcastSlot({
        therapist_name: newSlot.therapistName || newSlot.therapist_name || 'Dr. Hendra / Specialist',
        service_name: newSlot.serviceName || newSlot.service_name || 'Flash Destination Slot',
      ...
      await loadSalonsFromApi();
      showSuccess({
        id: 'Slot destinasi berhasil disiarkan secara live ke Gap Matcher feri!',
        en: 'Destination slot successfully broadcasted live to Ferry Gap Matcher!'
      }, {
      ...
      const msg = {
        id: e.message || 'Gagal menyiarkan slot destinasi.',
        en: e.message || 'Failed to broadcast destination slot.'
      };
...
export default useLokaBatamStore;
