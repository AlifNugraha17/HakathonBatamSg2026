<template>
  <header class="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b-2 border-slate-200 shadow-sm">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex items-center justify-between gap-3 h-14 sm:h-16 lg:h-18">
        ...
        >
          ...
          <div class="flex items-center gap-1.5">
            ...
            <span class="hidden sm:inline-block px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider bg-emerald-100 text-emerald-900 border border-emerald-300 shrink-0 font-mono">
              SG ⇄ Batam
            </span>
          </div>
        </div>

        <!-- Center Group: Desktop Navigation Links (Visible on lg+) -->
        <nav class="hidden lg:flex items-center gap-1 xl:gap-1.5 whitespace-nowrap overflow-hidden">
          
          <!-- Medis -->
          <a 
            @click.prevent="$emit('nav', 'medical')" 
            href="#" 
            class="px-2 xl:px-3 py-1.5 rounded-xl text-xs transition-all flex items-center gap-1 xl:gap-1.5 cursor-pointer shrink-0"
          ...
          >
            <span>🩺</span>
            <span>{{ t?.nav_medical || 'Medical' }}</span>
          </a>
          ...
          >
            <span>☕</span>
            <span>{{ lang === 'en' ? 'Tours' : (t?.nav_tourism || 'Kafe & Wisata') }}</span>
          </a>
          ...
          >
            <span>⛳</span>
            <span>{{ t?.nav_resorts || 'Resorts' }}</span>
          </a>
        ...
        <div class="flex items-center gap-1.5 sm:gap-2 shrink-0 whitespace-nowrap">
          ...
          <div class="flex items-center bg-slate-100 p-0.5 rounded-xl border border-slate-300 shadow-xs shrink-0">
            <button 
          ...
          <router-link
            to="/login"
            class="hidden sm:inline-flex items-center gap-1 px-2.5 sm:px-3 py-1.5 rounded-xl text-xs font-bold text-teal-800 bg-teal-50 hover:bg-teal-100 border border-teal-300 transition-all cursor-pointer shadow-xs shrink-0"
          >
            <span>🔐</span>
          ...
          >
            <span>⚡</span>
            <span class="hidden md:inline">{{ t?.nav_cta || 'Book Now' }}</span>
            <span class="md:hidden">{{ lang === 'en' ? 'Book' : 'Janji' }}</span>
          </button>

          <!-- Mobile Menu Hamburger Button (Visible on < lg) -->
          <button 
            @click="mobileMenuOpen = !mobileMenuOpen"
            class="lg:hidden w-9 h-9 rounded-xl bg-slate-100 border border-slate-300 text-slate-800 hover:bg-slate-200 flex items-center justify-center text-lg transition-colors cursor-pointer shrink-0"
            aria-label="Menu"
          >
    ...
    >
      <div v-if="mobileMenuOpen" class="lg:hidden bg-white border-b-2 border-slate-200 shadow-xl px-4 py-4 space-y-2">
        <div class="grid grid-cols-2 gap-2">
          ...
          <router-link 
            to="/login"
            class="p-2.5 rounded-xl text-xs font-bold text-left flex items-center justify-center gap-2 border bg-teal-50 text-teal-950 border-teal-300 transition-all col-span-2 shadow-xs"
          >
            <span>🔐</span>
            <span>Portal Login (Admin / Partner / Tourist)</span>
          </router-link>
