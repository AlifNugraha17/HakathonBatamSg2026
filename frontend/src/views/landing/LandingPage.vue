<template>
  <div class="min-h-screen bg-slate-50 text-teal-ink flex flex-col font-sans antialiased">
    ...
    <!-- Footer -->
    <footer class="bg-white border-t border-slate-200 py-8 mb-14 lg:mb-0">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between text-xs text-slate-500 gap-4">
        <div class="flex items-center space-x-2">
          <span class="font-extrabold text-teal-ink text-sm font-display tracking-tight">Loka<span class="text-teal-700">Batam</span></span>
          <span>© 2026 — Platform Wisata Medis & Travel Lintas Batas (SG ⇄ Batam)</span>
        </div>
        <div class="flex items-center space-x-3">
          <router-link to="/login" class="text-teal-700 hover:text-teal-900 font-bold bg-teal-50 hover:bg-teal-100 px-3 py-1 rounded-full border border-teal-200 transition-colors">
            🔐 Portal Login
          </router-link>
          <span class="text-emerald-700 font-mono font-bold bg-emerald-50 px-3 py-1 rounded-full border border-emerald-200">Vue 3 • Laravel 11 • PostgreSQL</span>
        </div>
      </div>
    </footer>
...
// Language State (Defaults to English 'en' initially, toggleable to 'id' in Navbar)
const lang = ref(localStorage.getItem('bp_lang') || localStorage.getItem('lokabatam_lang') || 'en')
const t = computed(() => translations[lang.value] || translations.en)
const setLang = (newLang) => {
  lang.value = newLang
  localStorage.setItem('bp_lang', newLang)
  localStorage.setItem('lokabatam_lang', newLang)
}
