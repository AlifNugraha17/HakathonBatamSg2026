<template>
  <div class="merchant-overview-view animate-fade-in">
    <!-- Performance KPI Row in English -->
    <div class="kpi-grid">
      <div class="kpi-card">
        <span class="kpi-label">Today's Revenue</span>
        <span class="kpi-val">IDR 1,450,000</span>
        <span class="kpi-sub">≈ SGD 122.30 (Settled via PayNow)</span>
      </div>

      <div class="kpi-card">
        <span class="kpi-label">Chair Occupancy Rate</span>
        <span class="kpi-val">80.0%</span>
        <span class="kpi-sub">4 of 5 Massage Chairs Booked</span>
      </div>

      <div class="kpi-card">
        <span class="kpi-label">Arrived Tourists</span>
        <span class="kpi-val">12 Guests</span>
        <span class="kpi-sub">Singapore (8), Malaysia (3), Aus (1)</span>
      </div>

      <div class="kpi-card">
        <span class="kpi-label">Hygiene Audit Score</span>
        <span class="kpi-val">98/100</span>
        <span class="kpi-sub">Zentura Verified Grade A</span>
      </div>
    </div>

    <!-- Active Flash Chairs and Pending Orders Summary -->
    <div class="split-overview-row">
      <!-- Flash Chairs Status -->
      <div class="overview-box">
        <div class="box-header">
          <h3 class="box-title">Active Flash Chairs</h3>
          <button class="link-btn" @click="$emit('switch-tab', 'slots')">Manage Broadcasts →</button>
        </div>

        <div class="slots-list">
          <div 
            v-for="slot in merchantSalon.flashSlots" 
            :key="slot.id" 
            class="slot-item-mini"
            :class="{ active: slot.isFlashActive }"
          >
            <div class="slot-mini-left">
              <span class="chair-tag">{{ slot.chair }}</span>
              <div class="slot-mini-info">
                <span class="srv-name">{{ slot.serviceName }} ({{ slot.durationMinutes }}m)</span>
                <span class="srv-therapist">Therapist: {{ slot.therapistName }} • {{ slot.time }}</span>
              </div>
            </div>
            <div class="slot-mini-right">
              <span class="slot-price">IDR {{ Number(slot.priceIdr).toLocaleString('id-ID') }}</span>
              <button 
                class="btn-toggle-slot"
                :class="slot.isFlashActive ? 'live' : 'off'"
                @click="toggleFlashSlot(slot.id)"
              >
                {{ slot.isFlashActive ? 'LIVE' : 'OFF' }}
              </button>
            </div>
          </div>
        </div>
      </div>

      <!-- Pending Orders List -->
      <div class="overview-box">
        <div class="box-header">
          <h3 class="box-title">Reservations Awaiting Confirmation</h3>
          <button class="link-btn" @click="$emit('switch-tab', 'orders')">View All Orders →</button>
        </div>

        <div v-if="pendingBookings.length > 0" class="orders-list">
          <div v-for="b in pendingBookings" :key="b.id" class="order-item-mini">
            <div class="order-mini-top">
              <span class="order-id">#{{ b.id }}</span>
              <span class="tourist-name">{{ b.touristName }} ({{ b.touristCountry }})</span>
            </div>
            <div class="order-mini-service">
              <span>{{ b.serviceName }} • {{ b.timeSlot }}</span>
            </div>
            <div class="order-mini-actions">
              <button class="btn-confirm-mini" @click="confirmBooking(b.id)">Accept & Confirm</button>
              <button class="btn-card-mini" @click="openTherapistCard(b)">AI Card</button>
            </div>
          </div>
        </div>

        <div v-else class="all-caught-up">
          <p>All incoming guest reservations have been confirmed.</p>
        </div>
      </div>
    </div>
  </div>
</template>

<script setup>
import { computed } from 'vue';
import { useZenturaStore } from '../../../composables/useZenturaStore';

defineEmits(['switch-tab']);

const { 
  merchantSalon, 
  merchantBookings, 
  toggleFlashSlot, 
  confirmBooking, 
  selectedTherapistCardBooking 
} = useZenturaStore();

const pendingBookings = computed(() => {
  return merchantBookings.value.filter(b => b.status === 'pending');
});

const openTherapistCard = (booking) => {
  selectedTherapistCardBooking.value = booking;
};
</script>

<style scoped>
.merchant-overview-view {
  display: flex;
  flex-direction: column;
  gap: 1.25rem;
}

.kpi-grid {
  display: grid;
  grid-template-columns: repeat(4, 1fr);
  gap: 1rem;
}

.kpi-card {
  padding: 1.35rem;
  border-radius: var(--radius-lg);
  background: #ffffff;
  border: 1px solid #e2e8f0;
  box-shadow: 0 4px 20px -2px rgba(30, 58, 138, 0.06);
  display: flex;
  flex-direction: column;
  transition: all 0.15s ease;
}

.kpi-card:hover {
  border-color: #93c5fd;
}

.kpi-label {
  font-size: 0.76rem;
  color: #1e3a8a;
  font-weight: 700;
  display: block;
  margin-bottom: 0.3rem;
  text-transform: uppercase;
  letter-spacing: 0.03em;
}

.kpi-val {
  font-size: 1.5rem;
  font-weight: 800;
  color: #0f172a;
  letter-spacing: -0.02em;
}

.kpi-sub {
  font-size: 0.72rem;
  color: #64748b;
  margin-top: 0.25rem;
  display: block;
}

.split-overview-row {
  display: grid;
  grid-template-columns: 1fr 1fr;
  gap: 1rem;
}

.overview-box {
  padding: 1.5rem;
  border-radius: var(--radius-lg);
  background: #ffffff;
  border: 1px solid #e2e8f0;
  box-shadow: 0 4px 20px -2px rgba(30, 58, 138, 0.06);
  display: flex;
  flex-direction: column;
}

.box-header {
  display: flex;
  justify-content: space-between;
  align-items: center;
  margin-bottom: 1.25rem;
}

.box-title {
  font-size: 1rem;
  color: #0f172a;
  font-weight: 700;
  margin: 0;
}

.link-btn {
  background: transparent;
  border: none;
  color: #2563eb;
  font-size: 0.78rem;
  font-weight: 700;
  cursor: pointer;
}

.link-btn:hover {
  text-decoration: underline;
}

.slots-list, .orders-list {
  display: flex;
  flex-direction: column;
  gap: 0.65rem;
}

.slot-item-mini {
  display: flex;
  justify-content: space-between;
  align-items: center;
  padding: 0.75rem 0.95rem;
  border-radius: var(--radius-sm);
  background: #f8fafc;
  border: 1px solid #e2e8f0;
}

.slot-item-mini.active {
  border-color: #bfdbfe;
  background: #ffffff;
}

.slot-mini-left {
  display: flex;
  align-items: center;
  gap: 0.75rem;
}

.chair-tag {
  font-size: 0.72rem;
  font-weight: 700;
  color: #1e3a8a;
  background: #eff6ff;
  border: 1px solid #bfdbfe;
  padding: 0.2rem 0.5rem;
  border-radius: 4px;
}

.srv-name {
  font-size: 0.84rem;
  font-weight: 700;
  color: #0f172a;
  display: block;
}

.srv-therapist {
  font-size: 0.72rem;
  color: #64748b;
  display: block;
}

.slot-mini-right {
  display: flex;
  align-items: center;
  gap: 0.65rem;
}

.slot-price {
  font-size: 0.84rem;
  font-weight: 700;
  color: #0f172a;
}

.btn-toggle-slot {
  padding: 0.25rem 0.6rem;
  border-radius: var(--radius-xs);
  font-size: 0.7rem;
  font-weight: 700;
  border: none;
  cursor: pointer;
}

.btn-toggle-slot.live {
  background: #1e3a8a;
  color: #ffffff;
}

.btn-toggle-slot.off {
  background: #e2e8f0;
  color: #64748b;
}

.order-item-mini {
  padding: 0.85rem;
  border-radius: var(--radius-sm);
  background: #f8fafc;
  border: 1px solid #e2e8f0;
  display: flex;
  flex-direction: column;
  gap: 0.4rem;
}

.order-mini-top {
  display: flex;
  justify-content: space-between;
  font-size: 0.82rem;
}

.order-id {
  font-weight: 700;
  color: #0f172a;
}

.tourist-name {
  font-weight: 700;
  color: #1e3a8a;
}

.order-mini-service {
  font-size: 0.76rem;
  color: #64748b;
}

.order-mini-actions {
  display: flex;
  gap: 0.5rem;
  margin-top: 0.25rem;
}

.btn-confirm-mini {
  flex: 1;
  padding: 0.4rem;
  border-radius: var(--radius-xs);
  background: #1e3a8a;
  color: #ffffff;
  font-weight: 700;
  font-size: 0.76rem;
  border: none;
  cursor: pointer;
}

.btn-confirm-mini:hover {
  background: #1d4ed8;
}

.btn-card-mini {
  padding: 0.4rem 0.75rem;
  border-radius: var(--radius-xs);
  background: #ffffff;
  border: 1px solid #cbd5e1;
  color: #334155;
  font-weight: 700;
  font-size: 0.76rem;
  cursor: pointer;
}

.all-caught-up {
  text-align: center;
  padding: 2.5rem 1rem;
  color: #64748b;
  font-size: 0.84rem;
}

@media (max-width: 1024px) {
  .kpi-grid {
    grid-template-columns: repeat(2, 1fr);
  }
  .split-overview-row {
    grid-template-columns: 1fr;
  }
}
</style>
