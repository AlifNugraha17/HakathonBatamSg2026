<template>
  <section class="py-14 bg-gradient-to-b from-white via-sky-50/50 to-slate-100 relative overflow-hidden border-b border-slate-200">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
      ...
      <div class="flex flex-col md:flex-row md:items-end justify-between mb-8">
        <div>
          <div class="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-teal-900 text-white text-xs font-extrabold uppercase tracking-wider mb-2.5 shadow-sm border border-teal-700">
            ...
            {{ t?.carousel_tag || 'Spot & Destinasi Favorit Batam' }}
          </div>
          <h2 class="text-2xl sm:text-3xl font-black text-teal-ink tracking-tight">
            {{ t?.carousel_title || 'Eksplorasi Destinasi Trending & Viral di Batam' }}
          </h2>
          <p class="text-sm text-slate-700 mt-1 max-w-2xl font-medium">
            {{ t?.carousel_desc || 'Pilihan liburan akhir pekan & layanan medis terbaik hanya 45 menit perjalanan kapal feri dari Singapura.' }}
          </p>
        </div>
      ...
      >
        <div class="grid grid-cols-1 lg:grid-cols-12 min-h-[400px] sm:min-h-[460px]">
          ...
          <div class="lg:col-span-5 p-6 sm:p-8 flex flex-col justify-between bg-gradient-to-br from-white via-sky-50/40 to-slate-50 border-l border-slate-200">
            ...
            <div class="pt-4 border-t border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div>
                <p class="text-[11px] text-slate-500 uppercase tracking-wider font-bold">{{ t?.carousel_cost || 'Estimasi Biaya' }}</p>
                <p class="text-xl font-black text-teal-800">
                  {{ currentSlide.priceLabel }}
                </p>
              </div>

              <!-- Colorful Vibrant Solid CTA Button -->
              <button 
        ...
        <div class="bg-slate-100 px-6 py-3 border-t border-slate-200 flex items-center justify-between">
          ...
          <div class="text-[11px] text-slate-700 font-black flex items-center gap-1.5">
            ...
            <span class="hidden sm:inline">{{ t?.carousel_auto || 'Auto-slide aktif • Sentuh atau klik titik navigasi' }}</span>
            <span class="sm:hidden">{{ t?.carousel_auto_short || 'Auto-slide aktif' }}</span>
          </div>
        </div>
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-3.5 mt-6">
        <div 
          v-for="(cat, idx) in activeQuickCategories" 
          :key="idx"
          @click="goToSlide(cat.slideIndex)"
          class="p-2.5 sm:p-3.5 rounded-2xl border-2 transition-all cursor-pointer flex items-center gap-2.5 sm:gap-3 shadow-sm hover:shadow-md hover:-translate-y-0.5"
          :class="cat.cardBg"
        >
          <div class="w-9 h-9 sm:w-11 sm:h-11 rounded-xl flex items-center justify-center text-lg sm:text-xl shadow-sm shrink-0" :class="cat.iconBg">
            {{ cat.icon }}
          </div>
          <div class="min-w-0">
            <h4 class="text-xs font-black text-teal-ink truncate">{{ cat.title }}</h4>
            <p class="text-[10px] sm:text-[11px] text-slate-600 font-medium truncate">{{ cat.subtitle }}</p>
          </div>
        </div>
      </div>
    ...
    return [
      {
        ...
        category: "Viral Cafe & Sunset",
        location: "Harbour Bay & Batam City",
        image: "/images/viral-cafe.jpg",
        desc: "Santorini-inspired waterfront cafe featuring artisan coffee, fresh pastries, and panoramic sunset views overlooking high-speed ferries crossing towards Singapore.",
        highlight: "🔥 Viral on TikTok & Reels",
        priceLabel: "S$ 6 - 15 (~Rp 80k)",
        actionType: "booking",
        actionText: "Reserve & Details",
        btnIcon: "☕",
        btnGradient: "from-sky-600 via-blue-600 to-indigo-700 hover:from-sky-700 hover:to-indigo-800 shadow-blue-600/30",
        perks: [
          "Prime open-sea sunset viewing spot",
          "Specialty artisan coffee & tropical mocktails",
          "5 mins walk from Harbour Bay Ferry Terminal"
        ]
      },
      {
        title: "Nongsa White Sands Beach Club & Resort",
        category: "Beach & Relaxation",
        location: "Nongsa Coast Gateway",
        image: "/images/viral-beach.jpg",
        desc: "Pristine white sands, turquoise waters, and private daybeds with scenic views of the Singapore Marina Bay skyline on the clear horizon.",
        highlight: "🌴 Coastal Hidden Gem",
        priceLabel: "S$ 25 - 60 (~Rp 350k)",
        actionType: "booking",
        actionText: "View Resort Package",
        btnIcon: "🌴",
        btnGradient: "from-emerald-600 via-teal-600 to-green-700 hover:from-emerald-700 hover:to-green-800 shadow-emerald-600/30",
        perks: [
          "Private beach with exciting watersports",
          "Exclusive international resort ambiance",
          "Near Nongsa Pura Ferry Terminal"
        ]
      },
      {
        title: "SG ⇄ Batam High-Speed Fast Ferry",
        category: "45-Minute Ferry Transit",
        location: "HarbourFront & Tanah Merah (SG)",
        image: "/images/hero-ferry.jpg",
        desc: "Comfortable daily express ferries with air-conditioned cabins, VIP seating, and departures every 30 to 60 minutes throughout the day.",
        highlight: "🚢 45-Minute Quick Transit",
        priceLabel: "From S$ 35 (Return ticket)",
        actionType: "ferry",
        actionText: "Check Ferry Schedule",
        btnIcon: "🚢",
        btnGradient: "from-amber-600 via-orange-600 to-amber-700 hover:from-amber-700 hover:to-orange-800 shadow-amber-600/30",
        perks: [
          "Operated by BatamFast, Majestic & Horizon",
          "30-day visa-free for Singapore passport holders",
          "Priority check-in & luggage handling"
        ]
      },
      {
        title: "RS Awal Bros Batam — Executive Health Screening",
        category: "Global Standard Medical Care",
        location: "Batam Centre Downtown",
        image: "https://images.unsplash.com/photo-1586773860418-d37222d8fce3?auto=format&fit=crop&w=1200&q=80",
        desc: "Comprehensive executive health screening, full laboratory diagnostics, ECG, Ultrasound, and specialist consults saving up to 70% vs Singapore.",
        highlight: "🏥 Save Up to 70%",
        priceLabel: "S$ 280 (SG Est: S$ 880)",
        actionType: "booking",
        actionText: "Book VIP Doctor",
        btnIcon: "🩺",
        btnGradient: "from-teal-600 via-cyan-700 to-teal-800 hover:from-teal-700 hover:to-cyan-800 shadow-teal-600/30",
        perks: [
          "Same-day lab & diagnostic results",
          "Foreign-trained senior medical specialists",
          "Door-to-door ferry terminal VIP pick-up"
        ]
      }
    ]
  }
...
const currentSlide = computed(() => activeSlides.value[currentIndex.value])

const activeQuickCategories = computed(() => {
  if (isEn.value) {
    return [
      { title: "Cafes & Sunset", subtitle: "Viral Harbour Bay spots", icon: "☕", iconBg: "bg-sky-200 text-sky-800", cardBg: "bg-sky-50 border-sky-300 hover:border-teal-600", slideIndex: 0 },
      { title: "Nongsa Beach", subtitle: "Beach club & resort", icon: "🌴", iconBg: "bg-emerald-200 text-emerald-800", cardBg: "bg-emerald-50 border-emerald-300 hover:border-emerald-600", slideIndex: 1 },
      { title: "Ferry Schedules", subtitle: "45 mins SG to Batam", icon: "🚢", iconBg: "bg-amber-200 text-amber-800", cardBg: "bg-amber-50 border-amber-300 hover:border-amber-600", slideIndex: 2 },
      { title: "Medical Care", subtitle: "Save up to 70%", icon: "🩺", iconBg: "bg-teal-200 text-teal-800", cardBg: "bg-teal-50 border-teal-300 hover:border-teal-600", slideIndex: 3 },
    ]
  }
