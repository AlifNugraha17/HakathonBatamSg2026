<template>
  <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center p-2.5 sm:p-4 bg-slate-950/75 backdrop-blur-md">
    <div class="bg-white w-full max-w-4xl rounded-2xl sm:rounded-3xl p-4 sm:p-7 border-2 border-slate-300 shadow-2xl relative max-h-[94vh] overflow-y-auto">
        ...
        <div class="flex items-center gap-3">
          ...
          <div>
            <div class="flex items-center gap-2">
              <h3 class="text-xl font-black text-teal-ink">{{ t?.ferry_modal_title || 'Jadwal Keberangkatan Feri Real-Time' }}</h3>
              <span class="flex h-2.5 w-2.5 relative">
                <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-500 opacity-75"></span>
                <span class="relative inline-flex rounded-full h-2.5 w-2.5 bg-emerald-600"></span>
              </span>
            </div>
            <p class="text-xs text-slate-600 font-medium">{{ t?.ferry_modal_sub || 'Rute Singapura (HarbourFront / Tanah Merah) ↔ Batam (Harbour Bay / Batam Centre / Nongsa / Sekupang)' }}</p>
          </div>
        </div>

        <div class="flex items-center space-x-2 bg-teal-900 text-white px-4 py-2 rounded-xl border border-teal-700 shadow-sm">
          <span class="text-xs text-teal-200 font-medium">{{ t?.ferry_live_time || 'Waktu Real-Time:' }}</span>
